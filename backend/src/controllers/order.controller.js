...
const calculateDiscount = async (code, cartTotal) => {
    const discount = await Discount.findOne({ code }).lean();
    if (!discount) {
        return 0; // No discount if the code doesn't exist
    }
    // validation 
...
export const createOrder = async (req, res) => {
    try {
        ...
        const { deliveryDetails, paymentMode, discountCouponCode } = req.body;
        ...
        if (minOrderValue && cartTotal < minOrderValue.minAmount) {
            return res.status(400).json({
                error: `minimum order value must more than or equals ${minOrderValue.minAmount}`,
            });
        }

        const discount = await calculateDiscount(discountCouponCode, cartTotal);
        const discountedTotal = cartTotal - discount;

        //getting applicable charges and totalBill
        const charges = await findApplicableCharges(req.user, discountedTotal);
        const totalBill = calculateTotalPayable(discountedTotal, charges);
